...
Pacific_Fishing_df$Lon<-Pacific_Fishing_df$Lon+360

asp<-SpatialPoints(Pacific_Fishing_df[,9:10],proj4string=CRS("+proj=longlat +datum=WGS84 +no_defs"))
...
All_Spatial_Fishing_Data

Pacific_Spatial_Fishing_Data<-cbind(Pacific_Spatial_Fishing_Data, Lat_Lon_Coords)


Toober<-st_write(Pacific_Spatial_Fishing_Data, "Test1.csv")


st


test<-st_contains(Spatial_Fishing_Data, Combined_Convention_Bounds)
plot(test)

plot(ms)

str(Combined_Convention_Bounds)


Test<-as(england, 'Spatial')

union
plot(Test)

spdf = SpatialPolygonsDataFrame(shape2)
plot(WCPFC)

library(rgdal)
library(rgeos)

writeOGR(obj=Test, dsn="tempdir", layer="torn", driver="ESRI Shapefile")
sf::st_write(england, driver="ESRI Shapefile")

rm(list=ls())
setwd("C:/Users/timot/Documents/GFw_Data")


Test<-read.csv("2012-2016_fishingeffortflag_1.0.csv")
